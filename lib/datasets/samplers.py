...
class FrameSampler(Sampler):
    ...
    def __init__(self, dataset):
        if hasattr(dataset, 'num_pose'):
            inds = np.arange(0, (dataset.num_pose))
            ni = (dataset.num_pose) // dataset.num_cams
        elif hasattr(dataset, 'total_ims'):
            inds = np.arange(0, len(dataset.total_ims))
            ni = len(dataset.total_ims) // dataset.num_cams
