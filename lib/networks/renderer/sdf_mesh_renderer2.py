...
class Renderer:
    ...
    def render(self, batch):
        ...
        if 'tbw' in batch and not cfg.vis_face_mesh and not cfg.vis_hand_mesh and False:
        ...
        with torch.no_grad():
            if False:
                ...
                from lib.utils.total.seg3d_lossless import Seg3dLossless
                mcube_res = 512
                self.resolutions = np.logspace(start=5,
                                               stop=np.log2(mcube_res),
                                               base=2,
                                               num=int(np.log2(mcube_res) - 4),
                                               endpoint=True) + 1.0
                self.resolutions = self.resolutions.astype(np.int16).tolist()
                b_min = batch['tbounds'][:,0].cpu().tolist()
                b_max = batch['tbounds'][:,1].cpu().tolist()
                tbounds = batch['tbounds'].cpu()
                center = tbounds.mean(dim=1)
                length = (tbounds[:,1] - tbounds[:,0]).max() + 0.5
                b_min = (center - length/2).tolist()
                b_max = (center + length/2).tolist()

                reconEngine = Seg3dLossless(
                    query_func=sdf_decoder,
                    b_min=b_min,
                    b_max=b_max,
                ...
                vertices, triangles = mcubes.marching_cubes(sdf.numpy(), cfg.mesh_th)

        mesh = trimesh.Trimesh(vertices, triangles)
        scales = ((length.cpu())/mcube_res).numpy()
