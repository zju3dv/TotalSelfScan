...
class Renderer:
    ...
    def batchify_rays(self, wpts, sdf_decoder, net, chunk, batch):
        ...
        for i in range(0, n_point, chunk):
            ...
            ret = sdf_decoder(wpts[i:i + chunk], batch)[:, :1]
    ...
    def render(self, batch):
        ...
        pose_pts = world_points_to_pose_points(pts, batch['R'], batch['Th'])

        # sampling points for blend weight training
        if 'pbw' in batch and not cfg.vis_face_mesh and not cfg.vis_hand_mesh:
            bw = pts_sample_blend_weights(pose_pts, batch['pbw'], batch['pbounds'])
            tnorm = bw[:, -1]
        ...
        pts = pts[inside]
        sdf_decoder = self.net.calculate_wpts_sdf
        with torch.no_grad():
