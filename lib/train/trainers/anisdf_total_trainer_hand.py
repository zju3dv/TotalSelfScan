...
class NetworkWrapper(nn.Module):
    ...
    def forward(self, batch):
        ...
        if 'delta_poses' in ret:
            deltaPose_loss = torch.norm(ret['delta_poses'], dim=2).mean()
            scalar_stats.update({'deltaPose_loss': deltaPose_loss})
            loss += 0.01 * deltaPose_loss
        ...
        if batch['latent_index'].item() < cfg.num_trained_mask and 'msk_sdf' in ret and ret['msk_sdf'].shape[1] > 0 and not cfg.train_bgfg and cfg.train_part != 'combine':
            # 不对脸的图像处理，否则会出现凹痕
        ...
        if 'smooth_sdf' in ret:
            ...
            scalar_stats.update({'smooth_sdf_loss': smooth_sdf_loss})
