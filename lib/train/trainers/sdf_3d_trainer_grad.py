...
class NetworkWrapper(nn.Module):
    ...
    def forward(self, batch):
        ...
        sdf, gradients, gradients_fea = sdf_network(wpts, batch, type='all')
        # calculate normal
        gradients = gradients.squeeze()
        gradients_fea = gradients_fea.squeeze()
        # gradients = sdf_network.gradient(wpts, batch).squeeze()

        ret = {'sdf': sdf, 'gradients': gradients, 'grad_fea': gradients_fea}
        ...
        grad_loss = (torch.norm(gradients_fea, dim=1) - 1.0)**2
        grad_loss = grad_loss.mean()
        scalar_stats.update({'grad_fea_loss': grad_loss})
        loss += 0.1 * grad_loss
