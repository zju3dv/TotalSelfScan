...
class NetworkWrapper(nn.Module):
    def __init__(self, net):
        ...
        if cfg.fix_body:
            for param in self.net.body_net.parameters():
                param.requires_grad = False

        if cfg.fix_face:
            for param in self.net.face_net.parameters():
                param.requires_grad = False

        if cfg.fix_handl:
            for param in self.net.handl_net.parameters():
                param.requires_grad = False

        if cfg.fix_handr:
            for param in self.net.handr_net.parameters():
                param.requires_grad = False
    ...
    def forward(self, batch):
        ...
        if batch['latent_index'].item() < cfg.num_trained_mask and 'msk_sdf' in ret and batch['img_type'] != 1:
            # 不对脸的图像处理，否则会出现凹痕
