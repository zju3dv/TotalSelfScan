...
def cal_weigth(weights, steps):
    weights = torch.Tensor(weights)
    index = torch.where(steps >= weights[0])[0]
    index = torch.max(index)
    step_weight = weights[1][index]
    return step_weight
...
class NetworkWrapper(nn.Module):
    ...
    def forward(self, batch):
        ...
        if 'smooth_sdf' in ret and batch['iter_step'] > 10000:
            smooth_sdf = ret['smooth_sdf']
            smooth_sdf_loss = torch.norm(smooth_sdf, dim=1).mean()
            scalar_stats.update({'bsdf_loss': smooth_sdf_loss})
            smooth_sdf_weigth = cal_weigth(cfg.smooth_sdf_weight, batch['iter_step'])

            loss += smooth_sdf_weigth * smooth_sdf_loss
        ...
        if 'zero_sdf' in ret:
            zero_sdf = ret['zero_sdf']
            zero_sdf_loss = torch.abs(zero_sdf).mean()
            scalar_stats.update({'zero_sdf_loss': zero_sdf_loss})
            zero_sdf_weight = cal_weigth(cfg.zero_sdf_weigth, batch['iter_step'])
            loss += zero_sdf_weight * zero_sdf_loss
        if 'rgb_smooth_of_view_dir' in ret:
            rgb_smooth_of_view_dir = ret['rgb_smooth_of_view_dir']
            rgb_smooth_of_view_dir = torch.abs(rgb_smooth_of_view_dir).mean()
            scalar_stats.update({'rgb_noise_loss': rgb_smooth_of_view_dir})
            rgb_noise_weigth = cal_weigth(cfg.rgb_noise_weigth, batch['iter_step'])
            loss += rgb_noise_weigth * rgb_smooth_of_view_dir
