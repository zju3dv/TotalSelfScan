...
def pose_points_to_tpose_points(ppts, bw, A):
    ...
    A: n_batch, 24, 4, 4
    """
    num_joints = bw.shape[1]
    sh = ppts.shape
    bw = bw.permute(0, 2, 1)
    A = torch.bmm(bw, A.view(sh[0], num_joints, -1))
    A = A.view(sh[0], -1, 4, 4)
    pts = ppts - A[..., :3, 3]
...
def pose_dirs_to_tpose_dirs_rigid(ddirs, bw, A):
    """transform directions from the pose space to the T pose
    ddirs: n_batch, n_points, 3
    bw: n_batch, 24, n_points
    A: n_batch, 24, 4, 4
    """
    select_bone = bw.argmax(dim=1)[0]
    select_A = A[0, select_bone]
    R_inv = torch.inverse(select_A[..., :3, :3])
    pts = torch.einsum('ijk,bik->bij', R_inv, ddirs)
    return pts
...
def tpose_dirs_to_pose_dirs_rigid(ddirs, bw, A):
    """transform directions from the T pose to the pose space
    ddirs: n_batch, n_points, 3
    bw: n_batch, 24, n_points
    A: n_batch, 24, 4, 4
    ...
    select_A = A[0, select_bone]
    R = select_A[..., :3, :3]
    pts = torch.einsum('ijk,bik->bij', R, ddirs)
    return pts
