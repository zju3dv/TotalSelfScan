...
def invert_mask(msk, padding=0):

    height, width = msk.shape[:2]
    interhand_msk = np.zeros_like(msk)
    left_top = np.argwhere(msk == 1).min(axis=0) - padding
    if left_top[0] < 0:
        left_top[0] = 0
    if left_top[1] < 0:
        left_top[1] = 0
    right_bottom = np.argwhere(msk == 1).max(axis=0) + padding
    if right_bottom[0] > height - 1:
        right_bottom[0] = height - 1
    if right_bottom[1] > width - 1:
        right_bottom[1] = width - 1
