...
def grid_sample_3d(image, optical, mode='linear'):
...
if __name__ == "__main__":
    if False:
        ...
        grid = torch.rand(1, 1, 1, 2)
        start = time.time()
        output1 = grid_sample(image, grid)
        end = time.time()
        ...
        print('using {}'.format(end - start))
        print(grid_sample(image, grid))

        print(F.grid_sample(image, grid, padding_mode='border', align_corners=True))
    if True:
