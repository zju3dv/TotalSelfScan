...
class Visualizer:
    ...
    def visualize_image(self, output, batch):
        ...
        result_dir = os.path.join(cfg.result_dir, 'comparison_novel_pose'+'_{}'.format(cfg.motion_data))
        ...
        #     (img_pred[..., [2, 1, 0]] * 255))
        cv2.imwrite(
            '{}/{:06d}.jpg'.format(result_dir, frame_index),
            (orig_img_pred[..., [2, 1, 0]] * 255))
        # save posed mesh
        result_dir = os.path.join(cfg.result_dir, 'novel_pose_mesh')
        os.system('mkdir -p {}'.format(result_dir))
        from pytorch3d.io import IO
        posed_mesh = output['posed_mesh']
        # IO().save_mesh(posed_mesh.to('cpu'), '{}/posed_mesh_{}.ply'.format(result_dir, frame_index))
