  ...
  if obj is None or obj == '':
    ...
    for split in splits:
      ...
      if len(numbers) == 1:
        ...
        raise ValueError(f'Invalid format to parse!')
  else:
...
def interpolate(src_codes, dst_codes, step=5):
  ...
  """
  if src_codes.ndim < 2 or src_codes.shape != dst_codes.shape:
...
def mix_style(style_codes,
  ...
  if (style_codes.ndim < 3 or style_codes.shape[1] != num_layers or
  ...
  *code_shape], and `boundary` to be with shape [1, *code_shape] and unit norm.
  ...
  Raises:
    ValueError: If the input latent codes, boundary, or strength are with
      invalid shape.
  """
  if boundary.ndim < 2 or boundary.shape[0] != 1:
