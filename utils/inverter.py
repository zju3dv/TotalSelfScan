...
class StyleGANInverter(object):
  ...
  the target image satisfyingly. To this end, this class optimzie the latent
  ...
  (3) Adversarial loss from discriminator. (optional)
  (4) Regularization loss from encoder. (optional, but recommended for in-domain
      inversion)
  ...
  def __init__(self,
    ...
    Args:
      ...
      iteration: Number of iterations for optimization. (default: 1000)
      ...
      perceptual_loss_weight: Weight for perceptual loss. 0 disables perceptual
        loss. (default: 5e-5)
      adversarial_loss_weight: Weight for adversarial loss from discriminator.
        0 disables adversarial loss. (default: 1e-2)
    ...
    self.has_encoder = self.G.has_encoder
    self.run_device = self.G.run_device
    self.D = StyleGANDiscriminator(self.model_name, self.logger)
    if self.has_encoder:
    ...
    assert self.D.gan_type == self.gan_type
    if self.has_encoder:
    ...
    self.loss_adv_weight = adversarial_loss_weight
  ...
  def preprocess(self, image):
    ...
    if images.shape[2] == 1 and self.G.image_channels == 3:
      images = np.tile(images, (1, 1, 3))
    if images.shape[2] != self.G.image_channels:
      raise ValueError(f'Number of channels of input image, which is '
                       f'{images.shape[2]}, is not supported by the current '
  ...
  def get_init_code(self, image):
    ...
    if self.has_encoder:
      ...
      z = np.random.randn(1, *self.encode_dim)
    return z.astype(np.float32)

  def invert(self, image, num_viz=0):
    """Inverts the given image to a latent code.

    Baiscally, this function is based on gradient descent algorithm.
    ...
    for step in pbar:
      ...
      # Adversarial loss.
      if self.loss_adv_weight:
        adv_score = self.D.net(x_rec)
        loss_adv = torch.mean(_softplus(0 - adv_score))
        loss = loss + loss_adv * self.loss_adv_weight
        log_message += f', loss_adv: {_get_tensor_value(loss_adv):.3f}'

      # Regularization loss.
      if self.has_encoder and self.loss_reg_weight:
    ...
    return _get_tensor_value(z)[0], viz_results
